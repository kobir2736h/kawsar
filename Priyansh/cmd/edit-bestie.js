...
module.exports.onLoad = async () => {
  ...
  const imgPath = path.join(dirMaterial, "bestu.png");
  const url = "https://i.imgur.com/RloX16v.jpg";

  // Create directory if not exists using native fs
  if (!fs.existsSync(dirMaterial)) {
    fs.mkdirSync(dirMaterial, { recursive: true });
  }

  // Download file using axios if not exists
...
async function makeImage({ one, two }) {
  ...
  let batgiam_img = await jimp.read(path.join(__root, "bestu.png"));
