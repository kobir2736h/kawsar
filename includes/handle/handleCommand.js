...
module.exports = function ({ api, models, Users, Threads, Currencies }) {
    ...
    return async function ({ event }) {
        ...
        if (!command) {
            ...
            else return api.sendMessage(`𝐖𝐫𝐨𝐧𝐠 𝐂𝐨𝐦𝐦𝐚𝐧𝐝! 𝐓𝐫𝐲 ➤ ${checker.bestMatch.target}`, threadID);
        }
        ...
        if (command.config.hasPermssion > permssion) 
            return api.sendMessage(`📑Ye command 📝Sirf Mere Øwner Ko Hi Allow Hai ✋❌ "${command.config.name}"`, threadID, messageID);
        ...
        try {
            ...
            if (DeveloperMode)
                console.log(`[ DEV MODE ] [ ${time} ] Command used: ${commandName} by user: ${senderID} at thread: ${threadID} | Args included: ${args.join(" ")} | Processing time: ${(Date.now()) - dateNow}ms`);
            return;
        } catch (e) {
            return api.sendMessage(`Having some unexpected error when using command ${commandName}, error: ${e}`, threadID);
        }
    };
};
